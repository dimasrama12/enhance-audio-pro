...
with open('src/components/WaveformPlayer.tsx', 'r', encoding='utf-8') as f:
    code = f.read()

# 1. Remove the Mount effect
code = re.sub(r'  // -- Mount effect: create WaveSurfer ONCE; infrastructure persists across file switches --\n.*?  // -- File-load effect', '  // -- File-load effect', code, flags=re.DOTALL)

# 2. Extract WaveSurfer.create and resize observer logic
injection = """      // 1. Create a fresh audio pipeline for THIS specific file
...
pattern = r"      const ws = wsRef\.current;\n      if \(!ws\) return; // mount effect hasn't fired yet \(shouldn't happen in practice\)\n\n      // 1\. Create a fresh audio pipeline for THIS specific file.*?      // 2\. Hot-swap WaveSurfer's internal media element to our fresh one\n.*?      }\n"
code = re.sub(pattern, injection, code, flags=re.DOTALL)

# 3. In the cleanup function of the File-load effect, call ws.destroy()
cleanup_target = """      // WaveSurfer NOT destroyed here — instance is reused for the next file switch.
      // It is destroyed only in the mount effect's cleanup on component unmount."""
cleanup_replacement = """      const ws = wsRef.current;
      if (ws) {
        try { ws.destroy(); } catch (err) {}
        wsRef.current = null;
      }"""
code = code.replace(cleanup_target, cleanup_replacement)

# 4. Add the style to the waveformRef div
div_target = """      <div
        ref={waveformRef}
...
div_replacement = """      <div
      ...
      />"""
code = code.replace(div_target, div_replacement)

with open('src/components/WaveformPlayer.tsx', 'w', encoding='utf-8') as f:
    f.write(code)
