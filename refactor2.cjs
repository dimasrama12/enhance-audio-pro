...
// 1. Remove the Mount effect (lines 350-468 approx)
const mountRegex = /  \/\/ ── Mount effect: create WaveSurfer ONCE; infrastructure persists across file switches ──[\s\S]*?  \/\/ eslint-disable-next-line react-hooks\/exhaustive-deps\n  }, \[\]\); \/\/ Mount only\n/;
code = code.replace(mountRegex, '');

// 2. Modify the file-load effect signature
const fileLoadRegex = /  \/\/ ── File-load effect: resets state and loads new file into the existing WaveSurfer ──\n  useEffect\(\(\) => \{/;
code = code.replace(fileLoadRegex, "  // ── File-load effect: creates a NEW WaveSurfer instance AND a fresh audio pipeline per-file ──\n  // We use `willChange: transform` on the container to prevent WebView2 from flashing black when the canvas is recreated.\n  useEffect(() => {");

// 3. Inject WaveSurfer creation inside initTimer
const initTimerRegex = /      const ws = wsRef\.current;\n      if \(!ws\) return; \/\/ mount effect hasn't fired yet \(shouldn't happen in practice\)\n\n      \/\/ 1\. Create a fresh audio pipeline for THIS specific file[\s\S]*?      \/\/ 2\. Hot-swap WaveSurfer's internal media element to our fresh one\n[\s\S]*?      } catch \(e\) \{\n        console\.error\('Failed to hot-swap media element:', e\);\n      }\n/;

const injection = [
"      // 1. Create a fresh audio pipeline for THIS specific file",
...
code = code.replace(initTimerRegex, injection);

// 4. Update the cleanup block inside the File-load effect
const cleanupRegex = /      \/\/ WaveSurfer NOT destroyed here — instance is reused for the next file switch\.\n      \/\/ It is destroyed only in the mount effect's cleanup on component unmount\./;
...
code = code.replace(cleanupRegex, cleanupInjection);

// 5. Add willChange and transform to the div
...
code = code.replace(divRegex, divInjection);
